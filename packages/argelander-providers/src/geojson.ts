...
export function stripToEnhancedGeoJson(strip: Strip, options: EnhancedGeoJsonOptions = {}): GeoJsonFeatureCollection {
  ...
  const left = strip.segments.map((s, i) => vertexValues(s.left, s, ref, pass ? (pass.left[i] ?? names.map(() => null)) : []));
  const right = strip.segments.map((s, i) => vertexValues(s.right, s, ref, pass ? (pass.right[i] ?? names.map(() => null)) : []));
  return {
    type: 'FeatureCollection',
 ...
 * freely) and gathers every other column into the sidecar unmodified.
 */
export function enhancedGeoJsonToStrip(fc: GeoJsonFeatureCollection): StripFromGeoJson {
  ...
  const [leftCoords, rightCoords] = feature.geometry.coordinates;
  ...
  const ref = typeof props.bodyRadiusKm === 'number' ? props.bodyRadiusKm : radiusKm(edgeVec(leftCoords[0]!, idx));
  ...
  for (let i = 0; i < leftCoords.length; i++) {
    ...
    const state = STATE_BY_CODE[numberAt(lp, idx.state ?? -1) ?? 2] ?? 'committed';
  ...
  const strip: Strip = {
    ...
    segments,
    provenance: importProvenance(props),
  };
...
function edgeVec(pos: EnhancedPosition, idx: Record<string, number>, ref?: number): Vec3 {
  ...
  return fromGeographic(lon, lat, elev, ref ?? 0);
}
...
function importProvenance(props: Record<string, unknown>): Provenance {
  return {
    ...
    // Unix-second epoch are rendering grade, sub-structure is not carried.
    generatedBy: asString(props.generatedBy, 'enhancedGeoJsonToStrip: envelope grade, geographic and unix-second rendering grade'),
...
export function stripToGeoJson(strip: Strip): GeoJsonFeatureCollection {
  ...
  if (zeroWidth) {
    geometry = { type: 'LineString', coordinates: strip.segments.map((s) => ll(s.left)) };
  } else {
    const forward = strip.segments.map((s) => ll(s.left));
    const backward = strip.segments.map((s) => ll(s.right)).reverse();
    geometry = { type: 'Polygon', coordinates: [[...forward, ...backward, forward[0]!]] };
  }
 ...
 * reversed right edge; a LineString becomes a zero-width track.
 */
export function geoJsonToStrip(fc: GeoJsonFeatureCollection, meta: { body: string; frame: string; instrumentId: string; bodyRadiusKm: number }): Strip {
  ...
  if (feature.geometry.type === 'LineString') {
    ...
    throw new RangeError('geoJsonToStrip expects a LineString or Polygon feature');
  }
