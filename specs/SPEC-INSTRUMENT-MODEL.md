...
One model file and one fixture strip per family live under `packages/argelander-core/fixtures/models/` and `fixtures/strips/`, both named `<family>.json`. The two anchors are regenerated by the conformance test (`UPDATE_FIXTURES=1` rewrites them from the samplers); the other nineteen are Phase 0 worked examples demonstrating each family's envelope and sub-structure signature per SPEC-STRIP section 3, authored at the same conformance scale. Every fixture validates against `validateStrip` and the strip JSON Schema; every model validates against `validateInstrumentModel`.
